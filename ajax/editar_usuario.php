...
if (version_compare(PHP_VERSION, '5.3.7', '<')) {
    ...
    require_once("../libraries/password_compatibility_library.php");
		...
		if (empty($_POST['firstname2'])){
            ...
            $errors[] = "Nombre de usuario vacio";
			...
			require_once ("../config/conexion.php");//Contiene funcion que conecta a la base de datos 
				...
				$user_id=intval($_POST['mod_id']);
                    ...
                    if ($query_update) {
                        ...
                        $consulta = "SELECT * FROM clubes WHERE CLUB ='" . $user_name . "'";
                        $UP = mysqli_query($con,$consulta);
                        $UP_da = mysqli_num_rows($UP);
                        if ($UP_da == 1) {
                        	$sql = "UPDATE clubes SET CLUB='".$user_name."', CORREO='".$user_email."'
                            WHERE CLUB='".$user_name."';";
                    	$query_update = mysqli_query($con,$sql);
                    	if ($query_update) {
                        	  echo "<script>location.reload();</script>";
                        }
                        }else{
                        	 mysqli_query($con, 'ALTER TABLE clubes AUTO_INCREMENT 1');
                    $sql = "INSERT INTO clubes (CLUB, CORREO)
                            VALUES('". $user_name . "', '" . $user_email . "');";
                    $query_new_club_insert = mysqli_query($con,$sql);
                    if ($query_new_insert) {
                        	  echo "<script>location.reload();</script>";
                        }
                        }
